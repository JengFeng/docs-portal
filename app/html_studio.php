...
function portal_render_html_studio(array $user): void
{
    ...
    $content = '<section class="html-studio" data-studio-root>'
        ...
        . '<button type="button" data-studio-mode="compare" data-studio-requires-generated disabled>大圖／HTML 比較</button>'
        ...
        . '<section data-studio-panel="source"><h2>先完成一張完整大圖</h2><p>手繪、矩形、文字與圖片都保留在同一張大圖；完成後再明確產生初始 HTML。</p><p class="studio-generation-status" data-studio-generation-status role="status">尚未產生 HTML；比較與標註功能已鎖定。</p><label>畫面目的<textarea data-studio-intent>建立一個近滿版、字體清楚的供水監測入口，包含狀態摘要、測站表格與主要操作。</textarea></label><button type="button" class="primary-button" id="studio-generate-html">產生初始 HTML 並開始比較</button></section>'
