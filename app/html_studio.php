...
function portal_render_html_studio(array $user): void
{
    ...
    $content = '<section class="html-studio" data-studio-root>'
        . '<div class="studio-intro"><div><p class="eyebrow">ADMINISTRATOR HTML STUDIO</p><h1>HTML 工作室</h1><p>可從完整大圖或既有 HTML 檔案開始；再使用真實預覽、標註、元件選取及程式碼修改完成頁面。</p></div><span class="studio-local-state" data-studio-save-state>瀏覽器草稿</span></div>'
        . '<div class="studio-toolbar" role="toolbar" aria-label="HTML 工作室模式">'
        . '<button type="button" data-studio-mode="source">起點大圖</button>'
        . '<button type="button" data-studio-mode="compare" data-studio-requires-generated data-studio-requires-comparison-source disabled>大圖／HTML 比較</button>'
        . '<button type="button" data-studio-mode="preview" data-studio-requires-generated disabled>真實預覽</button>'
        . '<button type="button" data-studio-mode="annotate" data-studio-requires-generated disabled>畫面標註</button>'
        . '<button type="button" data-studio-mode="inspect" data-studio-requires-generated disabled>選取元件</button>'
        . '<button type="button" data-studio-mode="code" data-studio-requires-generated disabled>HTML／CSS</button>'
        ...
        . '<aside class="studio-steps" aria-label="工作流程"><h2>形成流程</h2><ol><li class="is-active">選擇初始內容</li><li>產生或匯入 HTML</li><li>圈選與調整</li><li>驗證 RWD</li><li>下載版本</li></ol><button type="button" class="secondary-button" data-studio-reset>回到初始範例</button></aside>'
        ...
        . '<label class="studio-upload-button" for="studio-html-upload">匯入 HTML</label><input id="studio-html-upload" type="file" accept=".html,.htm,text/html" hidden>'
        ...
        . '<section data-studio-panel="source"><h2>選擇初始內容</h2><p>可先完成一張完整大圖再產生 HTML，也可直接匯入既有 .html／.htm 檔案開始編輯。</p><p class="studio-generation-status" data-studio-generation-status role="status">尚未產生或匯入 HTML；後續功能已鎖定。</p><label>畫面目的<textarea data-studio-intent>建立一個近滿版、字體清楚的供水監測入口，包含狀態摘要、測站表格與主要操作。</textarea></label><button type="button" class="primary-button" id="studio-generate-html">產生初始 HTML 並開始比較</button></section>'
        ...
        . '<section data-studio-panel="annotate" hidden><h2>畫面標註</h2><p>選擇圖示後直接在真實 HTML 上標示；圖面只顯示編號，右側說明會使用相同編號。</p>'
        . '<div class="studio-annotation-toolbar" role="toolbar" aria-label="標註工具"><button type="button" class="is-active" data-annotation-tool="rect" aria-pressed="true">框選</button><button type="button" data-annotation-tool="circle" aria-pressed="false">圓圈</button><button type="button" data-annotation-tool="arrow" aria-pressed="false">箭頭</button><button type="button" data-annotation-tool="number" aria-pressed="false">數字</button></div>'
        . '<p><strong data-studio-annotation-count>0 個標註</strong></p><div class="studio-annotation-list" data-studio-annotation-list><p class="muted" data-studio-annotation-empty>尚未建立標註。</p></div><button type="button" class="secondary-button" data-studio-clear-annotations>清除全部標註</button>'
        . '<div class="studio-instruction-builder"><h3>AI 修改指令</h3><p>所有編號說明完成後，產生一份可直接交給 AI 的修改指令。</p><button type="button" class="primary-button" data-studio-generate-instructions>產生 AI 修改指令</button><label>修改指令<textarea data-studio-instruction-output readonly placeholder="產生後會在這裡顯示完整指令。"></textarea></label><div class="studio-instruction-actions"><button type="button" class="secondary-button" data-studio-copy-instructions disabled>複製指令</button><button type="button" class="secondary-button" data-studio-download-instructions disabled>下載 TXT</button><button type="button" class="secondary-button" data-studio-download-json disabled>下載 JSON</button></div></div></section>'
